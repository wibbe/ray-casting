...
extern inline Vec2 vec2_make(f32 x, f32 y)
{
    Vec2 v;
    v.x = x;
    v.y = y;
...
extern inline f32 vec2_length(Vec2 v)
{
    return sqrtf((v.x * v.x) + (v.y * v.y));
}

extern inline Vec2 vec2_normalize(Vec2 v)
{
    f32 length = vec2_length(v);
    Vec2 ret = vec2_make(v.x / length, v.y / length);
    return ret;
}

extern inline Vec2 vec2_perp(Vec2 v)
{
    return vec2_make(-v.y, v.x);
}

extern inline f32 vec2_angle(Vec2 v)
{
...
extern inline Vec2 vec2_mul(Vec2 a, float t)
{
    return vec2_make(a.x * t, a.y * t);
}
...
extern inline float vec2_perp_dot(Vec2 a, Vec2 b)
{
...
#define pixel_draw(x, y, color) CORE->canvas.bitmap->pixels[x + (y * PUN_CANVAS_WIDTH)] = color
